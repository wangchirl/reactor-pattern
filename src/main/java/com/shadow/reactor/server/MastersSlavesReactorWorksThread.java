 ...
 * work 线程负责处理业务逻辑
 */
public class MastersSlavesReactorWorksThread {
    ...
    public static void main(String[] args) {
        int[] ports = new int[]{8088,8089,8090};
        List<AbstractReactor[]> subs = new ArrayList<>();
        AbstractReactor[] subReactors = new AbstractReactor[4];
        int j = 0;
        for (int i = 0; i < 4; i++) {
            subReactors[j++] = new SubReactor(i);
        }
        subs.add(subReactors);
        subReactors = new AbstractReactor[4];
        j = 0;
        for (int i = 10; i < 14; i++) {
            subReactors[j++] = new SubReactor(i);
        }
        ...
        for (int i = 0; i < ports.length; i++) {
            new MainReactor(ports[i],i).setSubReactor(subs.get(i));
        }
    }
    ...
    static class MainReactor extends AbstractReactor {
        ...
        public MainReactor(int port, int id) {
            try {
                this.id = id;
                ...
                super.thread = new Thread(this::run, "main-reactor-thread-" + this.id);
                super.start();
            } catch (Exception e) {
                e.printStackTrace();
                System.exit(1);
    ...
    static class SubReactor extends AbstractReactor {
        ...
        SubReactor(int i) {
            try {
                ...
                super.thread = new Thread(this::run, "sub-reactor-thread-" + i);
                super.start();
            } catch (Exception e) {
                e.printStackTrace();
            }
